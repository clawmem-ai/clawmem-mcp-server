...
function renderOrgInvitationLine(inv) {
  const id = inv && inv.id;
  const invitee = inv && inv.invitee;
  const login =
    (inv && typeof inv.login === "string" && inv.login) ||
    (invitee && typeof invitee.login === "string" && invitee.login) ||
    (invitee && typeof invitee.name === "string" && invitee.name) ||
    "(unknown)";
