...
async function ensureRoute() {
  ...
  const baseUrl = resolveBaseUrl(route);
  // Agent identities belong to the runtime, not to the working directory.
  // The service appends a collision-safe suffix, so sending the configured
  // prefix directly keeps account names short and avoids leaking project names.
  const prefix = resolveAgentPrefix();
