...
async function registerAgent({ baseUrl, prefixLogin, defaultRepoName }) {
  try {
    ...
    // Older self-hosted backends exposed agent registration under the
    // GitHub-compatible namespace. Keep this read-free compatibility fallback
    // after the extension endpoint, never before it.
    try {
      const res = await request(
        null,
        "agents",
...
async function wikiRequest(route, repo, suffix, init = {}) {
  const extensionRoute = { ...route, baseUrl: extensionApiBaseUrl(route && route.baseUrl) };
  try {
    return await request(extensionRoute, repoPath(repo, suffix), init);
  } catch (error) {
    // Keep read compatibility with older self-hosted GitHub-compatible
    // backends. The hosted ClawMem backend and every new write must use ext.
...
async function upsertWikiPage(route, repo, slug, params) {
  const extensionRoute = { ...route, baseUrl: extensionApiBaseUrl(route && route.baseUrl) };
  return request(extensionRoute, repoPath(repo, `wiki/pages/${encodeURIComponent(slug)}`), {
    method: "PUT",
    body: JSON.stringify({
      body: params.body,
...
function memoryBody({ detail, hash, kind, topics, validFrom }) {
  const body = String(detail || "").replace(/\r\n/g, "\n").trim();
  const meta = [
    "schema_version: clawmem/v2",
    `valid_from: ${validFrom || todayIsoDate()}`,
    "valid_to:",
    `memory_hash: ${hash}`
  ];
  if (kind) meta.push(`kind: ${kind}`);
  if (topics && topics.length > 0) meta.push(`topics: ${topics.join(", ")}`);
  return [
    "## Memory",
    "",
    body,
...
function extractMarkdownMemoryDetail(markdown) {
  const match = /^## Memory\s*\n+([\s\S]*?)(?=\n## |\s*$)/m.exec(String(markdown || "").trim());
  return match && match[1] ? match[1].trim() : "";
}

function extractLegacyYamlDetail(text) {
  const blockMatch = String(text || "").match(/detail:\s*\|-\n([\s\S]*)$/m);
  if (blockMatch) {
    return blockMatch[1]
      .split("\n")
      .map((line) => line.replace(/^  /, ""))
  ...
  const inlineMatch = String(text || "").match(/detail:\s*(.+)$/m);
...
function parseStoredMemoryBody(rawBody) {
  ...
  const legacyMeta = parseFlatYaml(visible);
  const meta = {
    ...legacyMeta,
    ...(hiddenMeta ? parseFlatYaml(hiddenMeta[1] || "") : {})
  };
  const detail = extractMarkdownMemoryDetail(visible)
    || extractLegacyYamlDetail(visible)
    || (meta.detail || "").trim()
    || visible;
  return { detail, meta };
}
...
function issueDetail(issue) {
  ...
  const parsed = parseStoredMemoryBody(body);
  return parsed.detail ? parsed.detail.trim() : clip(body.replace(/\s+/g, " ").trim(), 240);
}

function issueMemoryMeta(issue) {
  return parseStoredMemoryBody(String((issue && issue.body) || "")).meta;
}
...
async function storeMemory(route, repo, params) {
  ...
  const issue = await createIssue(route, repo, {
    title,
    body: memoryBody({ detail, hash, kind: kindSlug, topics: topicSlugs, validFrom: params.validFrom }),
    labels
  });
...
async function updateMemory(route, repo, memoryId, params) {
  ...
  const topicSlugs = params.topics !== undefined
    ...
    : topicsFromLabels(currentLabels);
  const currentMeta = issueMemoryMeta(current);
  const labels = memoryLabelsFor(kindSlug, topicSlugs);
  ...
  await updateIssue(route, repo, issueNumber, {
    ...
    body: memoryBody({
      ...
      kind: kindSlug,
      topics: topicSlugs,
      validFrom: params.validFrom || currentMeta.valid_from || currentMeta.date || todayIsoDate()
    })
  });
...
const ORG_INVITATION_ROLE_MAP = { member: "direct_member", owner: "admin" };

async function createOrgInvitation(route, org, { inviteeLogin, role, teamIds, expiresInDays } = {}) {
  const body = {
    invitee_login: inviteeLogin,
    role: ORG_INVITATION_ROLE_MAP[role] || role || "direct_member"
  };
