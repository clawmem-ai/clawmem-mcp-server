...
function sanitizeRecallQueryInput(text) {
  if (!text || typeof text !== "string") return "";
  const a = stripLeadingInboundMetadata(text).trimStart();
  const b = stripLeadingMessageIdHints(a).trimStart();
  const c = stripLeadingEnvelope(b).trimStart();
  const d = stripTrailingSystemHints(c).trimStart();
  return stripLeadingSenderPrefix(d).trimStart();
}
...
function buildRecallSearchText(rawQuery) {
  ...
  return truncateRecallQuery(cleaned, MAX_RECALL_QUERY_CHARS);
}
